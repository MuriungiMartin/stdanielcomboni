...
const Blog = () => {
    ...
    const displayedArticles = sortedArticles.slice(currentIndex, currentIndex + 3);

    return (
        <div className="container mx-auto lg:pt-16 md:pt-8 sm:pt-16">
            <div className="flex justify-center items-center">
                <h1 className="text-3xl font-bold mb-4 lg:pt-16 md:pt-8 sm:pt-16">Read Our Articles</h1>
            </div>
                ...
                {displayedArticles.map((article) => (
                    <div key={article.id} className="bg-white p-4 shadow-md">
                        <div className="flex justify-center pb-8">
                            <img src={article.imgSrc} alt="Article" className="h-40 w-fit" />
                        </div>
                        ...
                        <button onClick={() => toggleArticle(article.id)}>
                            {expandedArticles.includes(article.id) ? 'Show Less' : 'Read More'}
                        </button>
                    </div>
                ))}
            </div>
